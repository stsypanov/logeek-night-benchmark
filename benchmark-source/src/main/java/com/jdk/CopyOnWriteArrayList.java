...
public class CopyOnWriteArrayList<E>
    ...
    private boolean addIfAbsent(E e, Object[] snapshot) {
        synchronized (lock) {
            ...
            Object[] newElements = java.util.Arrays.copyOf(current, len + 1);
            newElements[len] = e;
            setArray(newElements);
            return true;
        }
    ...
    boolean bulkRemove(Predicate<? super E> filter, int i, int end) {
        ...
        if (i < end) {
            ...
            final Object[] newElts = java.util.Arrays.copyOf(es, es.length - deleted);
            int w = beg;
            for (i = beg; i < end; i++)
    ...
    void sort(Comparator<? super E> c, int i, int end) {
        // assert Thread.holdsLock(lock);
        final Object[] es = getArray().clone();
        java.util.Arrays.sort(es, i, end, (Comparator<Object>)c);
        setArray(es);
    }
